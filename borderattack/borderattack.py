...
class BorderAttack(AutoAttack):

    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
                 attacks_to_run=[], version='standard', is_tf_model=False,
                 device='cuda', log_path=None, n_restarts=1, n_iter=100, border_size=1, model_name=None, losses=None, losses_t=None):
        ...
        self.losses=losses,
        self.losses_t=losses_t

        if not self.is_tf_model:
            from .border_APGD import BorderAPGD, BorderAPGD_targeted

            self.bapgd = BorderAPGD(self.model, n_restarts=1, n_iter=n_iter, verbose=False,
                eps=eps, norm=self.norm, eot_iter=1, rho=.75, seed=self.seed,
                device=self.device, logger=self.logger, model_name=model_name, losses=losses)

            self.bapgd_t = BorderAPGD_targeted(self.model, n_restarts=1, n_iter=n_iter, verbose=False,
                eps=eps, norm=self.norm, eot_iter=1, rho=.75, seed=self.seed,
                device=self.device, logger=self.logger, model_name=model_name, losses=losses_t)
    ...
    def run_standard_evaluation(self,
        ...
        with torch.no_grad():
            ...
            for attack in attacks_to_run:
                ...
                if attack == 'bapgd':
                    self.logger.log(f'Evaluating with the loss combination: {self.losses}')
                elif attack == 'bapgd-t':
                    self.logger.log(f'Evaluating with the loss combination: {self.losses_t}')
                ...
                for batch_idx in range(n_batches):
                    ...
                    if attack == 'bapgd':
                        # apgd on dlr loss
                        self.bapgd.loss = 'combined'
                        self.bapgd.seed = self.get_seed()
                        adv_curr = self.bapgd.perturb(x, y) 

                    elif attack == 'bapgd-t':
                        # apgd on dlr loss
                        self.bapgd_t.loss = 'combined'
                        self.bapgd_t.seed = self.get_seed()
                        adv_curr = self.bapgd_t.perturb(x, y) 
    ...
    def set_version(self, version='standard'):
        ...
        if version == 'standard':
            self.attacks_to_run = ['bapgd', 'bapgd-t']
